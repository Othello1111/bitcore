...
We're working hard to make *bcore* the most powerful JavaScript library for working with bitcoin. Our goal is to have *bcore* be a library that can be used by anyone interested in bitcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in bcore that any change must adhere.
...
When possible, bcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of bcore methods. For example, calling `JSON.stringify` on an bcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'bcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the bcore object.

DO:
```javascript
var bcore = require('../');
var PublicKey = bcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add bcore git@github.com:bitcoin-cored/bcore.git
git pull --rebase bcore master
```
...
```
Finally go to [github.com/bitcoin-cored/bcore](https://github.com/bitcoin-cored/bcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of bcore.

If you have any questions feel free to post them to
[github.com/bitcoin-cored/bcore/issues](https://github.com/bitcoin-cored/bcore/issues).
